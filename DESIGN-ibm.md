...
## Urban Flow — Gray 100 Dark Theme (project override)

**Urban Flow** applies this Carbon system in the official **Gray 100 dark theme** (not the
white marketing theme above). Rationale: the whole page is dark so the site chrome (Hero,
About, dashboard UI) flows visually into the dark contour-terrain visualization canvas. All
structural Carbon rules still hold — flat 0px corners, one blue accent, IBM Plex Sans weight
300 display / 400 body with `letter-spacing: 0.16px`, hairlines-not-shadows.

The live token values are in `src/styles/tokens.css` (CSS custom properties) — that file is
the implementation source of truth; this section documents the intent.

### Surface (Gray 100 layering)
| Token | Value | Use |
|---|---|---|
| `--bg` | `#161616` | Page canvas + deck.gl clear color |
| `--layer-01` | `#262626` | Cards, panels, inputs |
| `--layer-02` | `#393939` | Nested / raised, secondary button |
| `--layer-03` | `#525252` | Nested-in-nested, hovers |

### Text
| Token | Value | Use |
|---|---|---|
| `--text-primary` | `#f4f4f4` | Headlines, primary body |
| `--text-secondary` | `#c6c6c6` | Secondary body, sub-headlines |
| `--text-helper` | `#8d8d8d` | Captions, helper, meta |

### Border
| Token | Value | Use |
|---|---|---|
| `--border-subtle` | `#393939` | Section dividers, card hairlines |
| `--border-strong` | `#6f6f6f` | Emphasized borders |

### Accent (IBM Blue on dark)
The one rule that changes on dark: **interactive text uses Blue 40, not Blue 60**, for contrast.
| Token | Value | Use |
|---|---|---|
| `--link` | `#78a9ff` (Blue 40) | Links, eyebrows, interactive text |
| `--link-hover` | `#a6c8ff` (Blue 30) | Link hover |
| `--button-primary` | `#0f62fe` (Blue 60) | Primary button fill (unchanged) |
| `--focus` | `#ffffff` | Focus ring (white on dark, per Carbon) |

### Do (dark-theme additions)
- Keep the primary button at Blue 60 `#0f62fe`; only *text-as-link* shifts to Blue 40.
- Build card/panel hierarchy by stepping `--bg` → `--layer-01` → `--layer-02`, plus 1px
  `--border-subtle` hairlines. Never drop shadows.
- Let the visualization canvas sit directly on `--bg` `#161616` with no seam.

### Don't
- Don't reintroduce the white marketing theme for Urban Flow surfaces.
- Don't use Blue 60 for link text on dark — it fails contrast; that's what Blue 40 is for.
