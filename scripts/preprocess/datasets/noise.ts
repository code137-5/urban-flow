...
/**
 * Noise (소음, dB) — 2023 yearly median per S-DoT sensor, resampled from ~900
 * scattered sites onto the grid by IDW (see sensors.ts). Same shape as the
...
export const noiseJob: DatasetJob = {
  id: 'noise',
  cellMeters: CELL_METERS,
  aggregation: 'mean',
  clip: false,
  padMeters: PAD_METERS,
  toCells: (bounds) => sensorsToCells(sensorsPath('noise'), 'noise_median', bounds, CELL_METERS),
}
